...
namespace STM32 {
    ...
    private:
        ...
        static bool waitEvent(I2CFlags flag) {
            ...
            while (!(REG->SR1 & flag)) {
                ...
                if (REG->SR1 & (I2CFlags::ACKNOWLEDGE_FAILURE | I2CFlags::BUS_ERROR | I2CFlags::ARBITRATION_LOST)) {
                    REG->SR1 &= ~(I2CFlags::ACKNOWLEDGE_FAILURE | I2CFlags::BUS_ERROR | I2CFlags::ARBITRATION_LOST); // Clear error flags
                    return false;
                }
        ...
        static inline void clearAddress() {
            (void)REG->SR1; (void)REG->SR2;       // Clear ADDR
        }
    public:
        ...
        static void enable(bool enableDMA = false) {
            ...
            REG->CCR = (REG->CCR & ~0xCFFFu) | (1 << 15) | (1 << 14) | (4u << 0);

            REG->TRISE = (REG->TRISE & ~0x3Fu) | (300 * 48) / 1000 + 1; // set Trise = MAX_RISE_TIME * BUS_FREQ + 1

            REG->CR1 |= 1u; // set EN to true
        }

        static bool writeRegister(uint8_t i2c_addr, uint8_t reg_addr, const uint8_t* data, const uint32_t size) {
            // START Transaction
            REG->CR1 |= 1 << 8;
